...
///
/// The flat `scores`/`frequencies`/`tree_sizes` fields are kept for backward
/// compatibility with existing callers. New callers should consume the
/// `instances` map, which mirrors the per-instance, per-tier breakdown
/// proposed in Mooncake RFC #1403 (kvcache-ai/Mooncake#1403):
